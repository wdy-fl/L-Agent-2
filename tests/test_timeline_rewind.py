...
def _run_agent(store: SQLiteTimelineStore, session_id: str, branch_id: str, user_input: str):
...
class TestRewindContextCorrectness:
    def test_rewind_includes_user_message_excludes_assistant(self):
    ...
    def test_rewind_does_not_modify_checkpoint_before_history(self):
        """Messages before the fork point are never modified."""
        store = SQLiteTimelineStore(":memory:")
        session = create_session_with_default_branch(store)
        branch_id = session.active_branch_id

        _run_agent(store, session.session_id, branch_id, "AAA")
        _run_agent(store, session.session_id, branch_id, "BBB")
...
class TestBranchStructureSharing:
    def test_new_branch_does_not_copy_messages(self):
    ...
    def test_collect_visible_merges_parent_and_own(self):
        """After rewind + new turn, collect_visible shows parent + own messages."""
        store = SQLiteTimelineStore(":memory:")
        session = create_session_with_default_branch(store)
        branch_id = session.active_branch_id

        _run_agent(store, session.session_id, branch_id, "original")
    ...
    def test_resume_after_rewind(self):
        """Resume on rewind branch returns correct merged context."""
        store = SQLiteTimelineStore(":memory:")
        session = create_session_with_default_branch(store)
        branch_id = session.active_branch_id

        _run_agent(store, session.session_id, branch_id, "turn1")
        _run_agent(store, session.session_id, branch_id, "turn2")
        ...
        _run_agent(store, session.session_id, new_branch_id, "diverged")
